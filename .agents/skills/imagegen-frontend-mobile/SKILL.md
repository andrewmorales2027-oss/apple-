...
description: REQUIRES AN IMAGE-GENERATION TOOL. Generates premium mobile app screen concepts and flows for iOS, Android, and cross-platform, presented in clean phone mockups. Enforces safe-area awareness, logical screen-to-screen flow, a locked design bible across the set, readable type, controlled palettes, and tactile texture. Generates images only; writes no code. If no image tool is available, say so rather than substituting hand-rolled SVG.
---
