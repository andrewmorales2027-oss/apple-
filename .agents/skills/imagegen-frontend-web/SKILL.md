...
description: REQUIRES AN IMAGE-GENERATION TOOL. Generates premium website design reference images, ONE separate horizontal image per section (8 sections means 8 images, never a single tall board). Enforces composition variety, background-image confidence, varied hero scale, a narrative concept spine, and one consistent palette across the set. Generates images only; writes no code. If no image tool is available, say so rather than substituting hand-rolled SVG.
---
