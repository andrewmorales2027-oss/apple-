...
This is a vocabulary, not a library. The agent should KNOW these pattern names to communicate about them, design with them in mind, and reach for them when the design read calls for them. **These are names only: the Block Library (Section 12) is an unpopulated schema, so there are no stored implementations to retrieve.** Build the pattern from sections 3 through 9, using the canonical skeletons in 5.A, 5.B, and 5.C where they apply.
...
**Status (local install note):** the schema below is defined, but **no blocks ship with this skill** and no `blocks/` directory exists. Section 10 is therefore a vocabulary with no implementations behind it: when it names a pattern, you are building it from the principles in sections 3 through 9, not retrieving a spec. Do not tell the user a block library exists, and do not go looking for one. The canonical implementations that DO exist in this file are the three skeletons in sections 5.A, 5.B, and 5.C.

If you write a block, follow this schema and place it under this skill's own directory.

### 12.A File Location
```
<this-skill-dir>/blocks/
  hero/
    asymmetric-split.md
