---
name: image-to-code
description: REQUIRES AN IMAGE-GENERATION TOOL. Image-first website build: generate section reference images first, analyze them deeply for text, typography, spacing, buttons, and color, then implement the frontend faithfully to those references. Prefers one large image per section over a compressed board, regenerates unclear sections fresh rather than cropping, and bans cards-inside-cards and micro-UI clutter. Without an image tool this workflow cannot run; fall back to design-taste-frontend and its section 4.8 image strategy.
---
